...
/-
Since we know that w = z, we can deduce that z = w
by the application of the proven theorem that equality is symmetric.
-/
...
begin
<<<<<<< HEAD:src/submissions/practice_1.lean
  unfold prop_1,
...
/-
<<<<<<< HEAD:src/submissions/practice_1.lean
The introduction rule for a 'for all' is to just assume
that you are given an object, x, of an arbitrary type. If you
are given an arbitrary type that you had no control over choosing,
then you can deduce that the proposition is true for all x.

In the example, we would assume that x is an arbitrary type
and that, if all objects of that type have property, P, then x must have property, P. 
-/
=======
...
def successor_of_even_is_odd_alt : Prop := 
  ∀ (n : ℕ), ev n → odd (n+1)
...
Proof:
 If we assume that P, Q, and R are arbitrary propositions,
and that we have a proof of 'P ∧ (Q ∧ R)' by the application of ∧
and → introduction rules. We can construct a proof of the proposition
'(P ∧ Q) ∧ R' by applying the ∧ (and) introduction rule to a proof
of (P ^ Q) and a proof of R. To obtain a proof of (P ∧ Q), we have 
to obtain proofs of P and Q. Applying the ∧ elimination rule
to 'P ∧ (Q ∧ R)' gives you a proof of P and and proof of (Q ∧ R).
Applying the ∧ elimination rule to (Q ∧ R) gives a proof of Q and R.
QED.
