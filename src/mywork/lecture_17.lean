...
def evens_union_ods1 : set ℕ := evens ∪ ods 
...
def evens_intersect_empty1 : set ℕ := { n : ℕ | ev n ∧ n ∈ empte }

--contains only even #s
def evens_intersect_complete : set ℕ := { n : ℕ | ev n ∧ true }
def evens_intersect_complete1 : set ℕ := { n : ℕ | ev n ∧ n ∈ complete }
