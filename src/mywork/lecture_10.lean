...
--Ex false quod libet (Ex falso) (from false, anything)
/-
...
/-
Challenge: No proposition can be both true and false. 

You can never have a proof of something and not have a proof of it
at the same time.
-/
...
begin
  assume P,
  apply not.intro, -- ¬(P ∧ ¬P) is equivalent to (P ∧ ¬P) → false, not neccessary
  assume pandnotp, --assume proof of (P ∧ ¬P)
  have p := pandnotp.left, --proof of P
  have notp := pandnotp.right, --proof of ¬P
  have f := notp p, -- apply proof of ¬P, which means P → false, to proof of P
  exact f, --which then gives you a proof of false, which is applied here to proof theorem
